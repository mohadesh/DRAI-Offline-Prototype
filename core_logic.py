...
def _get_actual_scaler(scaler_target, target_col_name):
    """
    Resolve the per-target scaler intelligently.
    Prevents the catastrophic error of applying the Carbon scaler to MD.
    """
    ...
    for k in keys:
        if target_col_name in k or k in target_col_name:
            return scaler_target[k]

    # 3. Hardcoded fallbacks based on typical naming conventions
    ...
    # 4. Positional fallback (usually MD is first, C is second)
    if len(keys) >= 2:
...
def _preprocess_target_like_training(series, target_name):
    """
    Match training logic: ffill -> bfill -> mean.
    CRITICAL FIX: Never fallback to 0.0. Use physical baselines (MD: ~92.0, C: ~2.0)
    """
    s = series.copy().ffill().bfill()
    if s.isna().any():
        mean_val = s.mean()
        # If mean is also NaN (entirely empty series)
        if pd.isna(mean_val):
            # Smart baseline fallback to prevent model shock
...
def _get_scaled_target_series(df, target_name, is_md, scaler_target, safe_freq):
    ...
    # Safely preprocess target avoiding 0.0 trap
    ...
    # 2. 2D Fallback for joint scalers
    try:
        ts_df = pd.DataFrame(index=df_clean.index)

        if TARGET_MD_COL in df_clean.columns:
            ts_df[TARGET_MD_COL] = _preprocess_target_like_training(df_clean[TARGET_MD_COL], TARGET_MD_COL)
        else:
            ts_df[TARGET_MD_COL] = 92.0

        if TARGET_C_COL in df_clean.columns:
            ts_df[TARGET_C_COL] = _preprocess_target_like_training(df_clean[TARGET_C_COL], TARGET_C_COL)
        else:
            ts_df[TARGET_C_COL] = 2.0
...
def run_inference_for_md_c(model_md, model_c, df_window, frequency="30T"):
    """
    Runs inference for MD and C (single-step n=1).
    """
    import joblib
    import pandas as pd
